...
public class MessegeController {
    @Autowired
    private AlarmService notificationService;
...
//log.debug(CC.WOO + "웹소켓.메시지컨트롤러 클라이언트가 보낸 메시지 :  " + escapedContent + CC.RESET);
	...
	@SendToUser("/topic/privateMessages") // /user/topic/PrivateMessage 주제로 메시지 보내기
	public ResponseMessage getPrivateMessage(final Message message
											, final Principal principal) throws InterruptedException {

		Thread.sleep(500); // 딜레이 0.5초 (시뮬레이션)
log.debug(CC.WOO +principal.getName() +" <-- 유저에게, 프라이빗 메시지 컨트롤러 메시지 :  " + message.getMessageContent() + CC.RESET);

		// 클라이언트에게 보낼 응답 메시지 객체를 생성하고, 이스케이핑한 메시지 내용을 저장하여 반환
		return new ResponseMessage(
			HtmlUtils.htmlEscape("프라이빗 메시지 to User " + principal.getName() + ": " + message.getMessageContent() )
		);
	}
